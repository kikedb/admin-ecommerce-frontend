...
const note = ref('')
const paymentTerms = ref('Ninguno')
...
const confirmPendingItem = () => {
  ...
  // No cerramos el modal, permitimos seguir agregando
}
...
const totalCost = computed(() => {
  return selectedItems.value.reduce((sum, item) => sum + ((item.orderQuantity || 0) * (item.orderCost || 0)), 0)
})
  ...
  <div class="space-y-6 max-w-7xl mx-auto pb-12">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Crear orden de compra</h1>
      </div>
      <div>
        <button @click="submitOrder" class="px-5 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 font-medium text-sm transition shadow-sm">
          Guardar y crear
        </button>
      </div>
    </div>
    ...
    <div class="flex gap-6 items-start">
      ...
      <div class="flex-1 space-y-6">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div class="flex items-center gap-4 mb-6">
            <div class="flex-1">
              <select v-model="distributor" class="w-full border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50">
                <option value="">Seleccionar distribuidor u origen</option>
                <option value="ext_1">Proveedor Externo</option>
                <option v-for="loc in locations" :key="'dist_' + loc.id" :value="loc.id">{{ loc.name }}</option>
              </select>
            </div>
          ...
          <div class="relative">
            ...
            <input v-model="searchQuery" type="text" class="block w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="Buscar productos para agregar">
              ...
              <ul class="py-1">
                <li v-for="item in searchResults" :key="item.id" @click="selectSearchResult(item)" class="px-4 py-3 hover:bg-gray-50 cursor-pointer border-b border-gray-100 last:border-0">
                  <div class="flex items-center">
                    <img :src="item.image" alt="" class="w-10 h-10 rounded object-cover mr-3">
                    <div>
                      <p class="text-sm font-medium text-gray-900">{{ item.product }}</p>
                      <p class="text-xs text-gray-500">SKU: {{ item.sku }} | Disponible: {{ item.locations.reduce((acc, loc) => acc + loc.disponible, 0) }}</p>
                    </div>
                  </div>
                </li>
              </ul>
            ...
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="border-b border-gray-200">
                  ...
                  <th class="pb-3 text-sm font-semibold text-gray-600 w-28 text-right">Costo unit.</th>
              ...
              <tbody class="divide-y divide-gray-100">
                <tr v-for="item in selectedItems" :key="'sel_' + item.uniqueId">
                  ...
                  <td class="py-4 text-right align-top">
                    <input v-model.number="item.orderQuantity" type="number" min="1" class="w-full border border-gray-300 rounded-md text-sm px-2 py-1 focus:ring-blue-500 focus:border-blue-500 text-right">
                  </td>
                  <td class="py-4 text-right align-top">
                    <div class="relative">
                      <span class="absolute inset-y-0 left-0 flex items-center pl-2 text-gray-500 text-sm">$</span>
                      <input v-model.number="item.orderCost" type="number" min="0" class="w-full border border-gray-300 rounded-md text-sm pl-6 pr-2 py-1 focus:ring-blue-500 focus:border-blue-500 text-right">
                    </div>
                  </td>
                  <td class="py-4 text-right align-top">
      ...
      <div class="w-80 space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h3 class="font-semibold text-gray-900 mb-4">Detalles de la orden de compra</h3>
          <div class="space-y-4">
            <div>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Fecha de llegada prevista (Máx 5 días)</label>
            ...
            <div>
              ...
              <input v-model="referenceNumber" type="text" class="w-full border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="#PO-xxxx">
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Nota para el distribuidor</label>
              <textarea v-model="note" rows="3" class="w-full border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500"></textarea>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Condiciones de pago</label>
              <select v-model="paymentTerms" class="w-full border-gray-300 rounded-lg text-sm bg-gray-50">
                <option>Ninguno</option>
              </select>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Moneda</label>
              <select v-model="currency" class="w-full border-gray-300 rounded-lg text-sm bg-gray-50">
                <option>CLP $</option>
              </select>
            </div>
          </div>
        ...
        <!-- Etiquetas -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h3 class="font-semibold text-gray-900 mb-4">Etiquetas</h3>
          <input v-model="tags" type="text" class="w-full border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="Añadir etiquetas">
        </div>
      </div>
    </div>
  </div>
